...
Class Captcha {
	...
	{ 		
		...
		$greyblack = ImageColorAllocate($image, 128, 128, 128); 
		
		
		
		//Make the background black 
		//ImageFill($image, 0, 0, 709, 150, $white); 	
		imagefilledrectangle($image, 0,0, 709, 99, $greyblack);
		//Add randomly generated string in white to the image
		ImageString($image, 5 ,5, 5, $this->SecurityCode, $white); 
		//imagestring ($im, $font, $x, $y, $string, $textColor);
		//Throw in some lines to make it a little bit harder for any bots to break 
		ImageRectangle($image,0,0, $this->width-1, $this->height-1, $greyblack); 
