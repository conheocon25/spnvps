...
class VideoLibrary extends Mapper implements \MVC\Domain\VideoLibraryFinder {
    ...
    function __construct() {
		...
		$tblVideoLibrary = "tinhthatvienphuong_video_library";
		$tblVideo = "tinhthatvienphuong_video";
		...
		$findByTopLibraryStmt = sprintf("
			SELECT * FROM %s VL 
			WHERE VL.id_category IN (select id FROM tinhthatvienphuong_category_video WHERE btype=4) 
			ORDER BY (select time from %s V where V.id=VL.id_video ) 
			DESC limit 8", $tblVideoLibrary, $tblVideo
		);
		$findByTopHistoryStmt = sprintf("
			SELECT * FROM %s VL 
			WHERE VL.id_category IN (select id FROM tinhthatvienphuong_category_video WHERE btype=5) 
			ORDER BY (select time from %s V where V.id=VL.id_video ) 
			DESC limit 8", $tblVideoLibrary, $tblVideo
		);
		...
		$findByKeyStmt = sprintf("
			...
			FROM 
				(
					tinhthatvienphuong_category_video CV 	
						INNER JOIN
					tinhthatvienphuong_video_library	VL	
						ON 
					CV.id = VL.id_category
				)
					INNER JOIN
				tinhthatvienphuong_video VIDEO
					ON
