					...
					<div class="accordion accordion-fix" id="accordion4">
						<div class="accordion-group box radius">
							<div class="accordion-heading b-title clearfix">
								<div class="b-t-icon-videotc">Video Được Nhiều Người Xem</div>
...
{
$page=$_GET['page'];
}$sodu_lieu = mysql_num_rows(mysql_query("select * from video_video vi,video_phanloai pl,video_categories ca where pl.maloai=ca.phanloai and ca.id=vi.category and vi.congbo=1 and ca.congbo=1 order by vi.views desc limit 100") ) or die(mysql_error());
$sotrang = $sodu_lieu/$baitren_mottrang;
$huuha=$page*$baitren_mottrang;
$result =mysql_query("select * from video_video vi,video_phanloai pl,video_categories ca where pl.maloai=ca.phanloai and ca.id=vi.category and vi.congbo=1 and ca.congbo=1 order by vi.views desc LIMIT {$huuha},{$baitren_mottrang} ") or die(mysql_error());
