...
$sodu_lieu = mysql_num_rows(mysql_query("select*from  video_categories ca,video_video vi,users us where ca.id=vi.category and us.tennguoidung=vi.user and us.quyen!=1 and vi.congbo='1' order by vi.id desc") ) or die(mysql_error());
$sotrang = $sodu_lieu/$baitren_mottrang;
$huuha=$page*$baitren_mottrang;
$result =mysql_query("select*from  video_categories ca,video_video vi,users us where ca.id=vi.category and us.tennguoidung=vi.user and  us.quyen!=1 and vi.congbo='1' order by vi.id desc LIMIT {$huuha},{$baitren_mottrang} ") or die(mysql_error());
...
if($i%2==0){
 ?>
<tr bgcolor="#CCCCCC">
		<th width="20"><a onClick="return confirm('Bạn có chắc chắn muốn xóa ?');"  href="?act=video_delnd&id=<?php echo $id ?>">Xóa</a></th>
		<th width="20"><a href="?act=video_csnd&id=<?php echo $id ?>">Sửa</a></th>
...
?>
											...
											<a href='?act=videond&page=<?php echo $page ?>'> <?php echo $page ?></a>
