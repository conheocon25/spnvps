...
<tr><td>Tên giảng sư/chuyên đề : </td><td><textarea name="giangsu" cols="100" rows=""><?php echo $name ?></textarea><br/><span class='style1'>Chú ý: Nếu muốn Danh mục nào đứng trước thì thêm vào số thứ tự vì sẽ sắp xếp theo thứ tự giảm dần.</span></td></tr>
...
<?php 							
require '../config.php';
$sql2="select*from giangsu gs,video_categories ca where gs.magiangsu=ca.giangsu and ca.id='$a'";
$kq2=mysql_query($sql2);
                                            if(mysql_num_rows($kq2)!=0)
                                            {
                                                while($row=mysql_fetch_array($kq2))
                                                {
						...
						<?php 							
require '../config.php';
$sql3="select* from giangsu";
$kq3=mysql_query($sql3);
                                            if(mysql_num_rows($kq3)!=0)
                                            {
                                                while($row=mysql_fetch_array($kq3))
                                                {
                                                   $magiangsu=$row["magiangsu"];
                                                  $tengiangsu=$row["tengiangsu"];
...
<?php 							
require '../config.php';
$sql4="select*from tinhthanh pl,video_categories ca where pl.matinh=ca.tinhthanh and ca.id='$a'";
$kq4=mysql_query($sql4);
                                            if(mysql_num_rows($kq4)!=0)
                                            {
                                                while($row=mysql_fetch_array($kq4))
                                                {
                                                   $matinh=$row["matinh"];
                                                  $tentinh=$row["tentinh"];
						...
						<?php 							
require '../config.php';
$sql5="select*from tinhthanh";
$kq5=mysql_query($sql5);
                                            if(mysql_num_rows($kq5)!=0)
                                            {
                                                while($row=mysql_fetch_array($kq5))
                                                {
                                                   $matinh=$row["matinh"];
                                                  $tentinh=$row["tentinh"];
...
<textarea name="editor1"><?php echo $gioithieu ?></textarea>
        <script>
...
if($_FILES['upload']['tmp_name']==""){
    $them=mysql_query("update video_categories set `sapxep`='$sapxep',`gioithieu`='$gioithieu',`giangsu`='$giangsupl',`tinhthanh`='$tinhthanh',name='$giangsu',img='$ten_anh',phanloai='$phanloai',noibac='$noibac',chuyende='$chuyende',congbo='$congbo',linkca='$kd' where id='$id'"); 
...
//Check MIME Type  
    ...
    //Print Error Message  
     echo "<script>alert('{$_FILES['upload']['name']} Chưa tải hình lên!');</script>";       
    //Debug  
   ...
   echo "Name: $name<br/ >Type: $type<br />Size: $size<br />Tmp: $tmp";               
    ...
    $them=mysql_query("update `video_categories` set `sapxep`='$sapxep',`gioithieu`='$gioithieu',`giangsu`='$giangsupl',`tinhthanh`='$tinhthanh',`name`='$giangsu',`img`='$duongdan',`phanloai`='$phanloai',`noibac`='$noibac',`chuyende`='$chuyende',`congbo`='$congbo',linkca='$kd' where `id`='$id'"); 
