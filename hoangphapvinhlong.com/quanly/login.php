...
if (isset($_POST['btnLogin'])){
	$uid = $_POST['txtUid'];
	$pwd = $_POST['txtPwd'];
	$uid=trim(mysql_real_escape_string($uid));
	$pwd=trim(mysql_real_escape_string($pwd));
	$sql = "select * from users where tennguoidung='$uid' and matkhau='$pwd' limit 1";
	if (mysql_num_rows(mysql_query($sql,$conn))>0) {
	$row = mysql_fetch_array(mysql_query($sql));
		$log = $uid;
		$quyen=$row["quyen"];
		if($quyen==1 or $quyen==3){
		
		$_SESSION['log'] = $uid;
		$_SESSION['quyen'] = $quyen;
		echo "<script>window.location='./'</script>";}else{echo"<script language='javascript' type='text/javascript'>";
                    echo"alert('Khong co quyen truy cap trang nay')";
                    echo"</script>";
