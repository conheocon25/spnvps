...
require '../config.php';
$a=$_GET["id"];
 $dulieu_sua="select*from albumvideo where id = '$a'";
...
<form name="f1" action="?act=albumvideo_cs&lenh=do" method="post" enctype="multipart/form-data">
...
<tr>
  ...
  <td><select name="chuyenmuc">
   <?php 							
require '../config.php';
$sql="select*from  theloaialbumvideo where id='$idtheloai'";
$kq=mysql_query($sql);
  ...
  <?php 							
require '../config.php';
$sql="select*from theloaialbumvideo where id !='$idtheloai' order by id desc";
$kq=mysql_query($sql);
                                            if(mysql_num_rows($kq)!=0)
                                            {
                                                while($row=mysql_fetch_array($kq))
...
if($_FILES['upload']['tmp_name']==""){
    ...
    $them=mysql_query("update `albumvideo` set `ten`='$ten',`idtheloai`='$chuyenmuc',`hinhbg`='$hinh',`linkal`='$kd',`noibac`='$noibac',`mota`='$mota' where `id`='$id'"); 
 echo "<script>window.location='./?act=albumvideo'</script>";
...
//Check MIME Type  
   ...
   echo "Name: $name<br/ >Type: $type<br />Size: $size<br />Tmp: $tmp";               
	...
	$them=mysql_query("update `albumvideo` set `ten`='$ten',`idtheloai`='$chuyenmuc',`hinhbg`='$duongdan',`linkal`='$kd',`noibac`='$noibac',`mota`='$mota' where `id`='$id'"); 
 echo "<script>window.location='./?act=albumvideo'</script>";
 }
 }
?>