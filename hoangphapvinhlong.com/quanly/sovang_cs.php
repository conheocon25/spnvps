...
require '../config.php';
$a=$_GET["id"];
 $dulieu_sua="select * from  sovang where id = '$a'";
...
<form name="f1" action="?act=sovang_cs&lenh=do" method="post" enctype="multipart/form-data">
...
if($_FILES['upload']['tmp_name']==""){
    $them=mysql_query("update `sovang` set `title`='$tieude',`img`='$ten_anh',`published`='$congbo',`fulltext`='$noidung',`created`='$created',`created_by`='$created_by',`hits`='$hits',`link`='$kd' where `id`='$id'"); 
	echo "<script>alert('Sửa tin thành công);</script>";
 echo "<script>window.location='./?act=sovang'</script>";}else{
...
//Check MIME Type  
   ...
   echo "Name: $name<br/ >Type: $type<br />Size: $size<br />Tmp: $tmp";               
    ...
    $them=mysql_query("update content_items set `title`='$tieude',`img`='$duongdan',`published`='$congbo',`fulltext`='$noidung',`created`='$created',`created_by`='$created_by',`hits`='$hits',`link`='$kd' where id='$id'"); 
 echo "<script>window.location='./?act=sovang'</script>";}
