...
class App extends Mapper implements \MVC\Domain\AppFinder {
    ...
    function __construct() {
        ...
        $this->selectAllStmt = self::$PDO->prepare( 
                            "select * from chuahungthien_app");
        $this->selectStmt = self::$PDO->prepare( 
                            "select * from chuahungthien_app where id=?");
		$this->findByStmt = self::$PDO->prepare( 
                            "select * from chuahungthien_app where alias=?");
		
		$this->notSignedStmt = self::$PDO->prepare("
							SELECT * 
							FROM `chuahungthien_app` 
							WHERE 
								id NOT IN ( 
        ...
        $this->insertStmt = self::$PDO->prepare( 
                            "insert into chuahungthien_app ( name, phone, address, email, banner, prefix, app, datecreate, dateupdate, dateactivity, type) values(?,?,?,?,?,?,?,?,?,?,?)");
		$this->deleteStmt = self::$PDO->prepare( 
                            "delete from chuahungthien_app where id=?");
		$this->checkStmt = self::$PDO->prepare( 
                            "select distinct id from chuahungthien_app where app=?");			
		$this->checkEmailStmt = self::$PDO->prepare( 
                            "select distinct id from chuahungthien_app where email=?");									
