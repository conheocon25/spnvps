...
class App extends Mapper implements \MVC\Domain\AppFinder {
    ...
    function __construct() {
        ...
        $this->selectAllStmt = self::$PDO->prepare( 
                            "select * from chuahonglien_app");
        $this->selectStmt = self::$PDO->prepare( 
                            "select * from chuahonglien_app where id=?");
		$this->findByStmt = self::$PDO->prepare( 
                            "select * from chuahonglien_app where alias=?");
		
		$this->notSignedStmt = self::$PDO->prepare("
							SELECT * 
							FROM `chuahonglien_app` 
							WHERE 
								id NOT IN ( 
        ...
        $this->insertStmt = self::$PDO->prepare( 
                            "insert into chuahonglien_app ( name, phone, address, email, banner, prefix, app, datecreate, dateupdate, dateactivity, type) values(?,?,?,?,?,?,?,?,?,?,?)");
		$this->deleteStmt = self::$PDO->prepare( 
                            "delete from chuahonglien_app where id=?");
		$this->checkStmt = self::$PDO->prepare( 
                            "select distinct id from chuahonglien_app where app=?");			
		$this->checkEmailStmt = self::$PDO->prepare( 
                            "select distinct id from chuahonglien_app where email=?");									
