					...
					<div class="accordion accordion-fix" id="accordion4">
						<div class="accordion-group box radius">
							<div class="accordion-heading b-title clearfix">
								<div class="b-t-icon-videotc">Album video mới nhất</div>
...
{
$page=$_GET['page'];
}$sodu_lieu = mysql_num_rows(mysql_query("select * from albumvideo al,theloaialbumvideo tl where tl.id=al.idtheloai order by al.id desc limit 100") ) or die(mysql_error());
$sotrang = $sodu_lieu/$baitren_mottrang;
$huuha=$page*$baitren_mottrang;
$result =mysql_query("select * from albumvideo al,theloaialbumvideo tl where tl.id=al.idtheloai order by al.id desc LIMIT {$huuha},{$baitren_mottrang} ") or die(mysql_error());
...
?>
											...
											<a href="index.php?frame=newvideovideo&page=<?php echo $page; ?>"><?php echo $page ?></a>
