...
require '../config.php';
$a=$_GET["id"];
 $dulieu_sua="select * from  content_items where id = '$a'";
...
<form name="f1" action="?act=tintuc_cs&lenh=do" method="post" enctype="multipart/form-data">
...
require '../config.php';
$sql="select*from content_categories where id =$idcategories";
$kq=mysql_query($sql);
...
if($_FILES['upload']['tmp_name']==""){
    $them=mysql_query("update `content_items` set `title`='$tieude',`img`='$ten_anh',`published`='$congbo',`fulltext`='$noidung',`created`='$created',`created_by`='$created_by',`hits`='$hits',`idcategories`='$giangsu',`shottext`='$shottext',`noibac`='$noibac',`link`='$kd' where `id`='$id'"); 
	echo "<script>alert('Sửa tin thành công);</script>";
 echo "<script>window.location='./?act=tintuc'</script>";}else{
...
//Check MIME Type  
   ...
   echo "Name: $name<br/ >Type: $type<br />Size: $size<br />Tmp: $tmp";               
    ...
    $them=mysql_query("update content_items set `title`='$tieude',`img`='$duongdan',`published`='$congbo',`fulltext`='$noidung',`created`='$created',`created_by`='$created_by',`hits`='$hits',`idcategories`='$giangsu',`shottext`='$shottext',`noibac`='$noibac',`link`='$kd' where id='$id'"); 
