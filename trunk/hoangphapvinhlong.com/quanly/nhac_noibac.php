...
-->
</style><input type="button" value="Thêm mới" name="btnNew" onclick="window.location='./?act=nhac_them'" class="button"><table border="1" cellpadding="2" bordercolor="#C9C9C9" width="100%">
	<tr>
...
$sodu_lieu = mysql_num_rows(mysql_query("select*from  baihat bh,album al where al.id=bh.idalbum and bh.noibac='1' order by bh.id desc") ) or die(mysql_error());
$sotrang = $sodu_lieu/$baitren_mottrang;
$huuha=$page*$baitren_mottrang;
$result =mysql_query("select*from  baihat bh,album al where al.id=bh.idalbum and bh.noibac='1' order by bh.id desc LIMIT {$huuha},{$baitren_mottrang} ") or die(mysql_error());
...
?>
											...
											<a href='?act=nhac_noibac&page=<?php echo $page ?>'> <?php echo $page ?></a>
