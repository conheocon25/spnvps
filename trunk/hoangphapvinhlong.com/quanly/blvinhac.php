...
$sodu_lieu = mysql_num_rows(mysql_query("select*from comment_video co,albumvideo vi where vi.id=co.news_id order by co.comment_id desc") ) or die(mysql_error());
$sotrang = $sodu_lieu/$baitren_mottrang;
$huuha=$page*$baitren_mottrang;
$result =mysql_query("select*from comment_video co,albumvideo vi where vi.id=co.news_id order by co.comment_id desc LIMIT {$huuha},{$baitren_mottrang} ") or die(mysql_error());
...
<tr bgcolor="#CCCCCC" height="50px">
		<th width="20"><a onClick="return confirm('Bạn có chắc chắn muốn xóa ?');"  href="?act=blvinhac_del&id=<?php echo $id ?>">Xóa</a></th>
...
?>
											...
											<a href='?act=blvinhac&page=<?php echo $page ?>'> <?php echo $page ?></a>
