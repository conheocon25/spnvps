...
-->
</style><input type="button" value="Thêm mới" name="btnNew" onclick="window.location='./?act=tintuc_them'" class="button"><table border="1" cellpadding="2" bordercolor="#C9C9C9" width="100%">
	<tr>
...
$sodu_lieu = mysql_num_rows(mysql_query("select*from  content_categories con,content_items coni,users u where con.id=coni.idcategories and coni.created_by=u.id order by coni.id desc ") ) or die(mysql_error());
$sotrang = $sodu_lieu/$baitren_mottrang;
$huuha=$page*$baitren_mottrang;
$result =mysql_query("select*from  content_categories con,content_items coni,users u where con.id=coni.idcategories and coni.created_by=u.id order by coni.id desc LIMIT {$huuha},{$baitren_mottrang} ") or die(mysql_error());
...
if($i%2==0){
 ?>
<tr bgcolor="#CCCCCC">
		<th width="20"><a onClick="return confirm('Bạn có chắc chắn muốn xóa ?');"  href="?act=tintuc_del&id=<?php echo $id ?>">Xóa</a></th>
		<th width="20"><a href="?act=tintuc_cs&id=<?php echo $id ?>">Sửa</a></th>
...
?>
											...
											<a href='?act=tintuc&page=<?php echo $page ?>'> <?php echo $page ?></a>
