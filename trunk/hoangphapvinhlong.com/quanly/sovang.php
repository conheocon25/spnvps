...
-->
</style><input type="button" value="Thêm mới" name="btnNew" onclick="window.location='./?act=sovang_them'" class="button"><table border="1" cellpadding="2" bordercolor="#C9C9C9" width="100%">
	<tr>
...
$sodu_lieu = mysql_num_rows(mysql_query("select*from sovang coni,users u where coni.created_by=u.id order by coni.id desc ") ) or die(mysql_error());
$sotrang = $sodu_lieu/$baitren_mottrang;
$huuha=$page*$baitren_mottrang;
$result =mysql_query("select*from sovang coni,users u where coni.created_by=u.id order by coni.id desc LIMIT {$huuha},{$baitren_mottrang} ") or die(mysql_error());
...
if($i%2==0){
 ?>
<tr bgcolor="#CCCCCC">
		<th width="20"><a onClick="return confirm('Bạn có chắc chắn muốn xóa ?');"  href="?act=sovang_del&id=<?php echo $id ?>">Xóa</a></th>
		<th width="20"><a href="?act=sovang_cs&id=<?php echo $id ?>">Sửa</a></th>
...
?>
											...
											<a href='?act=sovang&page=<?php echo $page ?>'> <?php echo $page ?></a>
