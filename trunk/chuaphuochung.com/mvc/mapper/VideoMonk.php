...
class VideoMonk extends Mapper implements \MVC\Domain\VideoMonkFinder {
    ...
    function __construct() {
		...
		$tblVideoMonk = "chuaphuochung_video_monk";
		$tblVideo = "chuaphuochung_video";
		...
		$findByTopLocalStmt = sprintf("select *  from %s VM Where VM.id_monk =20 order by (select time from %s V where V.id=VM.id_video ) DESC limit 8", $tblVideoMonk, $tblVideo);
		$findByTop10Stmt = sprintf("select *  from %s limit 10", $tblVideoMonk);
		$findByUpdateTopStmt = sprintf("select *  from %s VM order by (select time from %s V where V.id=VM.id_video ) DESC limit 12", $tblVideoMonk, $tblVideo);
